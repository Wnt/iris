...
//!
//! ── ONE SOCKET (the merge hook, taken 2026-09-10) ──────────────────────
//! This module has NO listener of its own. `src/ctlsock.rs` owns the single
//! `mamectl/1` socket (`IRIS_CTL_SOCK`) — the banner, the seq framing, the
//! connection threads and the engine drain — and routes any verb it does not
//! own here through the two-function seam:
//!
//!     kh_ctl::verb_owned(verb)                   -> bool
//!     kh_ctl::dispatch(&mut Machine, verb, rest) -> Reply
//!
//! plus `caps_fragment()` in its HELLO caps list. The verbs are applied on
//! ctlsock's ENGINE thread, in wire order behind the pointer and key verbs, so
//! an `OK` still means "applied".
//!
//! It is one socket because streamhost gives a station exactly one:
//! `SH_MAMECTL_SOCK` is what `mame_sock.rs` connects to for the browser's
//! input and what `scripts/serve/reset-tile.sh` sends `LOADST golden` down.
//! The earlier `IRIS_KH_CTL_SOCK` standalone listener existed only so this
//! plane could be proven before the input plane landed; it is gone, and so is
//! the knob.
//!
...
fn binary_id() -> Result<BinaryId, String> {
    ...
    let bytes = std::fs::read(&path).map_err(|e| format!("read {}: {}", disp, e))?;
    Ok(BinaryId {
        path: disp,
